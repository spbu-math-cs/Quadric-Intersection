...
def similarity_transformed(data1, data2, feature1, feature2, alpha, dist_type="dot_product"):
    """Consider data normed to 1. Features have the shape of len(data).
       Returns similarity array of shape (len(data1), len(data2))"""
    feature1_full = np.repeat(feature1, len(feature2)).reshape((len(feature1), len(feature2)))
    if dist_type == "dot_product":
        similarity = np.dot(data1, data2.T) - alpha * np.outer(feature1, feature2)
    if dist_type == "max":
        similarity = np.dot(data1, data2.T) - alpha * np.maximum(feature1_full, feature2)
    if dist_type == "min":
        similarity = np.dot(data1, data2.T) - alpha * np.minimum(feature1_full, feature2)
    if dist_type == "fraction":
        similarity = np.dot(data1, data2.T) / (1 + alpha * np.minimum(feature1_full, feature2))
    if dist_type == "multiplication":
        similarity = np.dot(data1, data2.T) * (1 - alpha * np.minimum(feature1_full, feature2))
    if dist_type == "sum":
        similarity = np.dot(data1, data2.T) - alpha * (feature1_full - feature2)
...
class IrBigData():
    ...
    parameters : dict of paramters:
        similarity_type : str, can be one of two values: dot_product, with_features
        ...
        protocol : str, either data_distractors2 or data_no_disractors
    ...
    def __init__(self, data, data_features, labels, parameters, distractors=None, distractor_features=None):
        self.data = data
    ...
    def get_distances_transformed(self, dist_type="dot_product", alpha=0.):
        if self.params["similarity_type"] == "dot_product":
            # distances here are scalar products
            self._distances = np.dot(self.data, self.data.T)
            self._distances_distractors = np.dot(self.data, self.distractors.T)

        if self.params["similarity_type"] == "quadrics":
            self._distances = similarity_transformed(self.data, self.data, self.data_features,
                                                    self.data_features, alpha, dist_type=dist_type)
            self._distances_distractors = similarity_transformed(self.data, self.distractors, 
                                                                self.data_features, self.distractor_features, 
                                                                alpha, dist_type=dist_type)
    ...
    def final_func(self):
        self.inv_dict_ = calculate_inv_dict_(self.labels)
        print("1")
        # get distances (similarity) between points
        self.get_distances_transformed(dist_type=self.params["dist_type"], alpha=self.params["alpha"])
        print("2")
        self.prepare_pairs()
        # manually calculate threshold s_f_
        self.s_f_ = manual_roc_score(np.hstack((self._distances_true, self._distances_false)), 
                            self._ds_issame, self.params["fpr_threshold"])[0]
        self.ir_with_distractors()
        self.print_info()


    def prepare_pairs(self, dist_type="dot_product", alpha=0.):
        self.pairs_true_ = []
        # compose true pairs
        for label in self.inv_dict_:
            self.pairs_true_.extend(list(itertools.permutations(self.inv_dict_[label], r=2)))
        
        # get distances for true and false pairs
        self._distances_true = np.array([self._distances[pair] for pair in self.pairs_true_])
        ...
        self._distances_false = np.hstack((self._distances_false, self._distances_distractors.flatten()))
        self._ds_issame = np.hstack(([1 for i in range(len(self._distances_true))], 
                                    [0 for i in range(len(self._distances_false))]))

        
    def check_dimension(self):
        assert len(self.data) == len(self.data_features) == len(self.labels)
        if isinstance(distractor, numpy.ndarray):
        ...
        for index, pair in tqdm(enumerate(self.pairs_true_)):
            ...
            if extra_info:
                ...
                if not condition_1:
                    ...
                    self.errors_distractors_dict_[pair] = self._distances[pair] < self._distances_distractors[pair[0]]
